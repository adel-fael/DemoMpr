...
public class ImageAdapter {
	...
	public static boolean writeDicomFile(
		...
		if (desc == null) {
			if (UID.ImplicitVRLittleEndian.equals(syntax.suitable)
					|| UID.ExplicitVRBigEndianRetired.equals(syntax.suitable)) {
		...
		try (
				DicomOutputStream dos = new DicomOutputStream(new BufferedOutputStream(new FileOutputStream(file)), dstTsuid)) {
	...
	private static void writeImage(
		...
		if (DicomOutputData.isNativeSyntax(dstTsuid)) {
			...
			DicomJpegWriteParam dicomJpegWriteParam = DicomJpegWriteParam.buildDicomImageWriteParam(dstTsuid);
						...
						if (!hasFragments && bulkData != null) {
							...
							if (numberOfFrame == 1) {
									...
									if (UID.RLELossless.equals(syntax.original)) {
										...
										for (int i = 1; i < nbFragments; i++) {
											...
											try (ByteArrayOutputStream out = new ByteArrayOutputStream(b.length())) {
												byte[] bytes = b.toBytes(pixeldataVR.vr, bigEndian);
												out.write(bytes, 0, bytes.length);
												try (SeekableInMemoryByteChannel channel = new SeekableInMemoryByteChannel(out.toByteArray())) {
								...
								if (fragmentsPositions.size() == numberOfFrame) {
									...
									int end = (frame + 1) >= fragmentsPositions.size()
											? nbFragments
