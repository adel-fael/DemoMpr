...
public class PrDicomObject implements PresentationStateLut {
	...
	public PrDicomObject(Attributes dcmPR, ImageDescriptor desc) {
		...
		if (dcmLut != null) {
			 ...
			 * @see <a
			 * href=
			 * "http://dicom.nema.org/medical/Dicom/current/output/chtml/part03/sect_C.11.6.html"
			 * >C.11.6
			 * Softcopy Presentation LUT Module</a>
			 * <p>Presentation LUT Module is always implicitly specified to apply over the
			 * full range
			 * of output of the preceding transformation, and it never selects a subset or
			 * superset of
		...
		if (StringUtil.hasText(seriesInstanceUID)) {
			for (Attributes refSeriesSeq : getReferencedSeriesSequence()) {
				if (seriesInstanceUID.equals(refSeriesSeq.getString(Tag.SeriesInstanceUID))) {
					List<Attributes> refImgSeq = DicomObjectUtil.getSequence(
