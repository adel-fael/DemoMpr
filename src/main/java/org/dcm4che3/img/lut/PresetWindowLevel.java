	...
	public static List<PresetWindowLevel> getPresetCollection(
		...
		if (!levelList.isEmpty() && !windowList.isEmpty()) {
			...
			for (int i = 0; i < levelList.size(); i++) {
				...
				PresetWindowLevel preset = new PresetWindowLevel(
		...
		if (!voiLUTsData.isEmpty()) {
			...
			for (int i = 0; i < voiLUTsData.size(); i++) {
				...
				PresetWindowLevel preset = buildPresetFromLutData(adapter, voiLUTsData.get(i), wl, explanation + dicomKeyWord);
				if (preset == null) {
					continue;
				}
				// Only set shortcuts for the two first presets
		...
		PresetWindowLevel autoLevel = new PresetWindowLevel(
				"Auto Level [Image]",
