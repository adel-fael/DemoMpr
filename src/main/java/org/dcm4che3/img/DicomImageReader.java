 ...
 *
 * <p>
 * Supports all the DICOM objects containing pixel data. Use the OpenCV native
 * library to read
...
public class DicomImageReader extends ImageReader {
	...
	/**
	 * Gets the stream metadata. May not contain post pixel data unless there are no
	 * images or the
	 * getStreamMetadata has been called with the post pixel data node being
	 * specified.
	 */
	@Override
	...
	public PlanarImage getPlanarImage(int frame, DicomImageReadParam param) throws IOException {
		...
		if (getImageDescriptor().hasPaletteColorLookupTable()) {
			if (dis == null) {
				out = DicomImageUtils.getRGBImageFromPaletteColorModel(
	...
	protected PlanarImage getRawImageFromFile(int frame, DicomImageReadParam param)
		...
		boolean rawData = pixeldataFragments == null
				|| type == TransferSyntaxType.NATIVE
				|| type == TransferSyntaxType.RLE;
		int dcmFlags = (type.canEncodeSigned() && desc.isSigned())
		...
		try {
			...
			if (rawData) {
				...
				MatOfInt dicomparams = new MatOfInt(
